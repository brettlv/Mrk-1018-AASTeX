...
\begin{abstract}
We present the multi-wavelength and long-term evolution of Changing-look AGN, Mrk~1018, especially during its fading phase, which shows rapid variability in X-ray and optical band. While Mrk~1018 shows strong correlation between ultraviolet and X-ray flux, the flux in ultraviolet varied more than that in X-ray as a whole and reversed in type transition period. The type transition timescale for Mrk~1018 is around 2 years or less, while we found the minimum timescale for variability can be as short as $\sim$ 100 days. The photon index($\Gamma$) indicating the hardness in X-ray and $\alpha_{OX}$ representing the relative strength of ultraviolet and X-ray, good probe of corona and accretion disk, both appear as "V-shape" when compared to luminosity, even though there is a wide gap. However, only weak variability appears in the radio band, makes it hard to understand the mechanism of radio emission.  
\end{abstract}
...
There are six filters in optical/ultraviolet band of \swift/UVOT: V,B,U,UVW1,UVM2 and UVW2. We use the tool \textit{uvotsource} to do the aperture photometry for all the observations of each filter. The source aperture radius is 5$\arcsec$ and the background is chosen in a blank region with much larger aperture radius. The results of the V and B filters are discarded, because the emission from the host galaxy is dominated in these two bands even in the bright phase \citep{2018MNRAS.480.3898N}. We discard the results of all filters after 2016, since the emission from the nucleus is comparable or less than the host galaxy \citep{2018MNRAS.480.3898N}, see Tab.~\ref{tab:tablealpha_ox} for details..  Milky Way extinction correction using \texorpdfstring{E(B$-V$) = 0.024}. mag \footnote{Retrieved via the NASA/IPAC Extragalactic Database (NED): \url{http://ned.ipac.caltech.edu/}} with extinction model from \citet{2007ApJ...663..320F} is adopted.  
