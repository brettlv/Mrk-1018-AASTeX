...
\begin{abstract}
We present the multi-wavelength and long-term variability of a Changing-Look active galactic nucleus (CL-AGN) Mrk~1018, which was previously found to change from type 1 to 1.9 between 2009 and 2015. There was a rapid decay of X-ray and UV flux during the type change, while the e-folding decay timescale is $\sim$ 800--1200 days.  We find a re-brightening flare within 98 days during the decay, during which the UV vs. X-ray flux follows evident positive correlation. We find that the spectral photon index ($\Gamma$) vs. the Eddington scaled X-ray luminosity ($L_\mathrm{2-10~ keV}/L_\mathrm{Edd}$) roughly follows a ``V-shape'' correlation that $\Gamma$-$L_\mathrm{2-10~ keV}/L_\mathrm{Edd}$ follows negative/positive correlation below/above the critical Eddington ratio $L_\mathrm{bol}/L_\mathrm{Edd}\sim$1\%. We find the source transited between the positive branch and the negative branch in the $\Gamma$-$L_\mathrm{2-10~ keV}/L_\mathrm{Edd}$ correlation during the re-brightening flare. We also find the fractional variability $\sim$ 23\% in X-ray band and $\sim$ 6\% in UV band on timescale of days when Mrk~1018 is at faint state during 2018. The scaled luminosity at frequency of 5 GHz in radio band declines by $\sim$ 20\%, and the radio spectral index becomes flat as the bolometric luminosity declines by around one order of magnitude during the type change. The timescale $\sim$ 98 days of the re-brightening flare without tracing the clockwise ``V-shape'' route is more consistent with the thermal timescale derived from the standard thin accretion disk. We require further simultaneous observations in optical and UV/X-ray band to understand the relation between AGN type and spectral state indicated by ``V-shape'' spectral evolution.
...
%\subsection{Is the optical type linked to the X-ray spectrum?}
%\label{sec:accretion mode}
...
We thank Chris Done for her friendly help with the usage of host galaxy model, and Dr Linhui Wu and Minhua Zhou for discussions on VLA data reduction.
...
\centering
    ...
    \caption{$L_\mathrm{2-10\,keV}/L_\mathrm{Edd}$ vs. $L_\mathrm{bol}/L_\mathrm{Edd}$. $L_\mathrm{bol}$ is estimated based on the model of \texttt{phabs*redden*(optxagnf+hostpol)}.}
    \label{fig:xray-bol}
\end{figure*}
